...
insert into movie (mo_num, mo_title, mo_contents, mo_director, mo_main_actor, mo_sub_actor, mo_opening_date, mo_genre, mo_runtime, mo_rating)
    values (1, "컨저링 3 - 악마가 시켰다.", "모든 것은 악마가 시켰다!", "마이클 차베즈", "베라 파미가, 패트릭 윌슨, 줄리안 일리아드", "", '2021-06-03', "호러,미스터리, 스릴러", 112, 15);
select * from movie;
...
/* 영화정보 확인하는 쿼리문 - 컨저링 3 */
select mo_contents from movie where mo_title like concat('%','컨저링 3','%');
select mo_contents from movie where mo_title like concat('%컨저링 3%');

/* 컨저링3의 오늘(2021-06-03) 상영시간을 확인하는 쿼리문 */
select sc_start_time from screen join movie on mo_num = sc_mo_num where mo_title like '%컨저링 3%' and sc_date like '2021-06-03%';
...
/* CGV청주성안길 컨저링 3 가 오늘 9:00에 있을 때 해당 영화의 선택 가능한 좌석을 확인 */
-- 해당 영화의 상영관시간 번호와 상영관 번호를 확인.
-- 해당 영화 예매 내역 중 예약된 좌석 현황.
-- 해당 영화의 예매 가능한 좌석 정보를 확인.

-- 해당 영화의 상영시간번호와 상영관 번호를 확인.
...
select mo_title from movie 
	where mo_num in 
    (select sc_mo_num from screen join reservation on sc_num = re_sc_num
		where re_me_id = 'abc123');
