...
/* 소녀시대의 멤버들을 검색하기 위한 쿼리문 
	singer 테이블(소녀시대), artist(멤버이름), member*/
select ar_name from singer
		join `member` on me_si_num = si_num
        join artist on ar_num = me_ar_num
        where si_name = '소녀시대';
...
/* 태연이 활동한 그룹들을 확인하는 쿼리문
   artist 테이블(태연), member 테이블(속한 그룹), singer 테이블(그룹명)*/
select si_name from artist
	join `member` on ar_num = me_ar_num
    join singer on si_num = me_si_num
	where ar_name like '%태연%';
    
/* 태연(가수)이 피처링한 노래들을 확인하는 쿼리문 
	signer 테이블 : 태연, sing 테이블 : 피처링 여부, source 테이블 : 노래 */
select so_title from sing 
	join singer on si_num = sn_si_num
    join `source` on so_num = sn_so_num
	where si_name like '%태연%' and sn_featuring = 'Y';

/* 소녀시대의 메인타이틀곡들을 확인하는 쿼리문 
   */ 
select so_title from singer
	join sing on si_num = sn_si_num
    join `source` on so_num = sn_so_num
