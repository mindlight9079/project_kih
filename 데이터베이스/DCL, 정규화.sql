/* DCL - 데이터 제어어
   ...
   revoke : 특정 데이터 베이스 사용자에게 권한 회수.
   commit : 트랜잭션의 작업이 정상적으로 완료됨을 알려줌.
   rollback : 트랜잭션 작업이 비정상적으로 완료되었을 때 복구.
*/
...
반정규화 : 정규화로 테이블이 너무 많아서 검색할 때 너무 많은 join이 필요한 경우 반정규화를 활용하여 테이블에 속성을 추가.      
*/
...
/* 소녀시대 3집앨범의 수록곡을 확인하기 위한 쿼리문
   list 테이블에 가수이름을 저장하는 li_si_name을 추가하여(반정규화) 쿼리문 작성 */
