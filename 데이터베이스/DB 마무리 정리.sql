...
- DCL
	- truncate : 테이블을 초기화
	- delete와 truncate를 이용하여 전체 튜풀을 삭제하는 경우 비교
		- delete를 이용하여 데이터를 삭제하면 auto_increment을 가지는 속성이 있는 경우 기존 값을 이어서 증가.
		- truncate를 이용하여 데이터를 삭제하면 auto_increment을 가지는 속성이 있는 경우 1부터 시작.
		- truncate는 전체를 한번에 삭제하지만 delete는 한행씩 삭제해서 전체를 삭제하기 때문에 delete가 느림.
		- 전체를 초기화하는 경우 truncate을 이용.
...
키
 ...
 - 기본키 : 후보키 중에 대표할 수 있도록 선정된 키, primary key
 - 대체키 : 기본키가 되지 못한 후보키, unique key
 ...
 DB 복습
 1. DB 설계(ERD) - 작은 규모의 시스템(과일과게에서 과일 판매시스템)부터 설계하는 연습을 하고, 이후에 규모가 큰 시스템을 설계하는 연습을 해야 함.
