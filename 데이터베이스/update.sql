...
- update문에 조건절을 생략하거나 조건절이 기본키에 대한 조건절이 아니면
  mysql 워크벤치에서는 안정성을 위해 update문을 실행하지 않음.
  실행하려면 워크벤치 기본 설정을 변경. (Edit > Preferences > SQL Editor)
  단, 워크벤치가 아닌 MySQL 8.0 Command Line Client로 접속해서 해당 쿼리를 실행하면 설정에 상관없이 실행 됨.
