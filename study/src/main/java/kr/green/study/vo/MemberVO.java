...
public class MemberVO {
	...
	//등급 비교가 안되면 -100, 권한이 같으면 0, 내가 권한이 낮으면 -1, 내가 권한이 높으면 1
	public int compareAuthority(MemberVO user) {
		...
		switch(authority) {
		case "USER" :
			if(user.getAuthority().equals("ADMIN") || user.getAuthority().equals("SUPER ADMIN"))
				return -1;
			return -100;
		case "ADMIN" :
			...
			return -100;	
		case "SUPER ADMIN" :
			if(user.getAuthority().equals("ADMIN") || user.getAuthority().equals("USER"))
				return 1;
			return -100;
		}
		return -100;
