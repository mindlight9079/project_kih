...
//		System.out.println();
	...
	/* 기능    : 배열 arr에 0번지부터 count개까지 확인하여 정수 num가 있는지 없는지 확인하여 알려주는 메소드 
	 * 매개변수 : 배열 arr, 갯수 count, 정수 num => int arr[], int count, int num
	 * 리턴타입 : 있는지 없는지 => boolean
	 * 메소드명 : contains
	 * 설명    : [1,2,3,4,5] 배열에서 처음부터 3개중에서 5이 있는지 없는지 확인. => 없음
	 * 			[1,2,3,4,5] 배열에서 처음부터 5개중에서 5이 있는지 없는지 확인. => 있음
	 */
	...
	/* 기능 : 배열 arr가 주어지면 주어진 배열에 min~max 사이의 랜덤한 수를 생성하여 중복되지 않게 저장하는 메소드
	 * 매개변수 : 배열 arr, 랜덤범위(min~max) => int arr[], int min, int max
	 * 리턴타입 : 없음 => void
	 * 메소드명 : init
	 */
	 ...
	 * 매개변수 : 배열 arr1, arr2 => int []arr1, int []arr2
	 ...
	 * 매개변수 : 배열 arr1, arr2 => int arr1[], int arr2[]
	 * 리턴타입 : 같은 자리가 아닌 수들 중에서 같은 숫자의 개수 => 정수 => int
	 * 메소드명 : getBall
	 * 설명    : [1 2 3] [3 2 4] => 1 => 같은 숫자의 개수 - 스트라이크의 수 => 2-1=1
	 */
