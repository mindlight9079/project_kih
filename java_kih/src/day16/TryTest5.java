...
public class TryTest5 {
	...
	public static void main(String[] args) {
		...
		System.out.print("두 정수와 산술 연산자를 입력해주세요 (예 : 1+2) : ");
	...
	public static double calculate(int num1, char op, int num2)  {
		...
		switch(op) {
			...
			case '*' :
				res = num1*num2; 		break;
			case '%' :
				//num2가 0인 경우 예외가 자동으로 발생되지만 if문과 throw를 쓰는 이유는 예외 메시지를 한글로 하고싶어서
				if(num2 == 0) {
					throw new ArithmeticException("0으로 나눌 수 없습니다.");
				}
				res = num1%num2; 		break;
			case '/' :
				//num2가 0이어도 실수/정수이어서 예외가 자동으로 발생하지 않아서 예외를 발생시키기 위해 if문과 throw를 사용
				if(num2 == 0) {
					throw new ArithmeticException("0으로 나눌 수 없습니다.");
				}
