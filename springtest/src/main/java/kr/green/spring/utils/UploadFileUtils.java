...
public class UploadFileUtils {
	/* 기능 : 파일 주소와 파일명과 파일이 주어지면 해당 파일주소에 업로드일을 기준으로 /년도/월/일 폴더를 만들어 해당 폴더에 파일을 업로드 하는 메소드
	 * 매개변수 : uploadPath - 업로드할 파일 경로, originalName - 실제 파일명
	 * 		    fileData - 업로드할 실제 파일 데이터가 있는 배열
	 * 리턴타입 : 저장된 파일경로와 파일명으로 된 문자열
	 * 예시 : uploadPath = "D:\\JAVA_TEST\\uploadfiles", originalName = "dog.jpg"
	...
	/* 기능 : 업로드할 파일 경로를 알려주면 해당 파일 경로에 오늘 날짜에 해당하는 폴더가 있는지 확인하여 없으면 폴더를 생성하고,
	 * 		 날짜 폴더 경로를 알려주는 메소드
	 ...
	 * 매개변수 : uploadPath - 업로드 경로 path- 폴더들
	 * 리턴타입 : 없음
	 */
	private static void makeDir(String uploadPath, String... paths) {
		//D:\\JAVA_TEST\\uploadfiles\\2021\\07\\14 
	...
	/* 기능 : 업로드 경로와 날짜 폴더 경로와 파일명이 주어지면 하나의 문자열로 반환하는 메소드
	 * 	 	 단, 경로에 \\대신 /로 
	 * */
