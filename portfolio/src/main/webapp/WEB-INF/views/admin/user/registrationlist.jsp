...
<body>
    ...
    <div class="management">Management</div>
        ...
        <div class="search">
            <form class="input-group float-right mb-3" action="<%=request.getContextPath()%>/admin/user/registrationlist">
              <select class="form-control mr-2" name="type">
                  ...
                  <option value="2" <c:if test="${pm.criteria.type == 2 }">selected</c:if>>카테고리</option>
              </select>
          ...
          <table class="table table-bordered">
            ...
            <tbody>
            <c:forEach items="${regi}" var="registration" varStatus="status">
              <tr>
                <td>${registration.re_code}</td>
                <td><a href="<%=request.getContextPath()%>/admin/user/registrationdetails?re_code=${registration.re_code}">${registration.re_title}</a></td>
                <td>${registration.re_price}</td>
                <td>${registration.re_amount}</td>
              </tr>
             </c:forEach>
            </tbody>
          </table>
	        <div class="regi-btn">
				<button class="btn btn-outline-secondary"><a href="<%=request.getContextPath()%>/admin/user/registration">판매가능도서등록</a></button>
	        </div>
              <ul class="pagination justify-content-center">
                  <c:if test="${pm.prev}">
                      <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/admin/user/registrationlist?page=${pm.startPage-1}&type=${pm.criteria.type}&search=${pm.criteria.search}">이전</a></li>
                  </c:if>
                  <c:forEach begin="${pm.startPage}" end="${pm.endPage}" var="index">
                      <li class="page-item <c:if test="${pm.criteria.page == index }">active</c:if>"><a class="page-link" href="<%=request.getContextPath()%>/admin/user/registrationlist?page=${index}&type=${pm.criteria.type}&search=${pm.criteria.search}">${index}</a></li>
                  </c:forEach>
      
                  <c:if test="${pm.next}">
                      <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/admin/user/registrationlist?page=${pm.endPage+1}&type=${pm.criteria.type}&search=${pm.criteria.search}">다음</a></li>
                  </c:if>
              </ul>
    </div>
<script>
